...
bool ReliableRouter::shouldFilterReceived(const meshtastic_MeshPacket *p)
{
    ...
    if (p->from == getNodeNum()) {
        ...
        if (old) {
            ...
            // marked as wantAck
            // This delivers inline and sniffReceived() stops the retransmissions itself, so nothing
            // more is needed here. See the commit message for why the old transport gate never ran.
